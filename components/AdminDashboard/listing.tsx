...
const AdminDashboard: Page = (props: any) => {
  ...
  const xyz =
    areas?.result?.length > 0
      ? areas.result.sort(
          (a: any, b: any) =>
            new Date(a.start_meeting_date).getTime() -
            new Date(b.start_meeting_date).getTime()
        )[0]?.start_meeting_date
      : undefined;
  const springDate =
    areas?.result?.length > 0
      ? areas.result.sort(
  ...
  const abc: any = xyz
    ? dayjs.tz(xyz, "America/New_York").valueOf()
    : undefined;
  ...
  const DashboardData = [
    {
      ...
      iconBackground:
        "linear-gradient(135deg, rgba(255, 171, 0, 0) 0%, rgba(255, 171, 0, 0.24) 97.35%)",
    ...
    },
    {
      ...
      title: (
        <span>
          {xyz2} Fall Member 2025(
          <Timmerday endDate={springDate} />)
        </span>
      ),
  ...
  const non_completed = state2?.filter(
    (res: any) => res?.is_form_completed == false
  );
  ...
  const dataSource = state1?.map((res: any, index: number) => {
    ...
    return {
      key: index + 1,
      name: capFirst(
        res?.firstname ? `${res?.firstname} ${res?.lastname}` : "N/A"
      ),
  ...
  const filteredArray = state2.filter(
    (item: any) => item.is_form_completed == true
  );
      ...
      return {
        ...
        name: res?.firstname
          ? `${validation.capitalizeFirstLetter(
              res?.firstname
            )} ${validation.capitalizeFirstLetter(res?.lastname)}`
          : "N/A",
        company: companyName,
        email: res?.email || "N/A",
        action: (
  ...
  const dataSource2 = non_completed?.map((res: any, index: number) => {
    ...
    return {
      ...
      name: res?.firstname
        ? `${validation.capitalizeFirstLetter(
            validation.capitalizeFirstLetter(res?.firstname)
          )} ${res?.lastname}`
        : "N/A",
      company: companyName || "N/A",
      action: (
              ...
              <ul className="m-0 list-unstyled d-flex gap-2">
                <li>
                  <Link href={`/admin/meetings/${res?.id}/view`}>
                    <Button type="primary" className="ViewMore primary">
                      <EyeOutlined />
                    </Button>
                  </Link>
                </li>
                ...
                return (
                  ...
                  >
                    <Link className="text-decoration-none" href={data.link}>
                      <Card
                        className="dashboard-widget-card text-center h-100 border-0"
              ...
              DashboardData2?.map((data: any, index: number) => {
                return (
                  ...
                  >
                    <Link className="text-decoration-none" href={data.link}>
                      <Card
                        className="dashboard-widget-card text-center h-80 border-0"
                        style={{ background: data.cardBackground }}
                      >
          ...
          <Col sm={24} md={24} lg={24} xxl={12}>
            <Card className="common-card">
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 mb-3">
                <Typography.Title level={4} className="m-0 fw-bold">
                  Complete Updates for {xyz1_meeting} 2025
                </Typography.Title>
              </div>
        ...
        {getUserdata?.is_admin == false ? (
          <Col sm={24} md={24} lg={24} xxl={12}>
            <Card className="common-card">
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 mb-3">
                <Typography.Title level={4} className="m-0 fw-bold">
                  Incomplete Updates for {xyz1_meeting} 2025
                </Typography.Title>
              </div>
              <div className="tabs-wrapper">
                {!state2.length ? (

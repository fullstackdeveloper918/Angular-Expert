...
const QuestionnairList = () => {
  ...
  const initialise = async (query: any) => {
    try {
      ...
      let res = await api.Questionnaire.listing(
        query,
        getUserdata.meetings.NextMeeting.id
      );
  ...
  const generatePdf = async () => {
    ...
    const blob = await pdf(
      <BussinessPdf state={state1} companyName={companyName} />
    ).toBlob();
  ...
  const generatePdfBusinessEvolution = async () => {
    ...
    const timestamp = new Date().toISOString().replace(/[-T:\.Z]/g, "");
    const blob = await pdf(
      <BusinessEvolutionPdf1 state={state1} companyName={companyName} />
    ).toBlob();
    const pdfUrl = URL.createObjectURL(blob);
    return { blob, pdfUrl, timestamp };
  };
  ...
  const sharePdf = async () => {
    ...
    const file = new File([blob], `Order_${timestamp}.pdf`, {
  ...
  const data = [
    ...
    {
      title: "Card 6",
      description: "SPRING 2025 MEETING PREPARATION",
      state: 5,
    },
  ...
  const getLinkForState = (state: any) => {
    switch (state) {
      case 0:
        return `${baseURL}/page2?${getUserdata?.user_id}&edit&questionnair`;
      case 1:
        return `${baseURL}/page3?${getUserdata?.user_id}&edit&questionnair`;
      case 2:
        return `${baseURL}/page4?${getUserdata?.user_id}&edit&questionnair`;
      case 3:
        return `${baseURL}/page5?${getUserdata?.user_id}&edit&questionnair`;
      case 4:
        return `${baseURL}/page6?${getUserdata?.user_id}&edit&questionnair`;
      case 5:
        return `${baseURL}/page7?${getUserdata?.user_id}&edit&questionnair`;
      case 6:
        return `${baseURL}/additional_questionnaire?${getUserdata?.user_id}&edit&questionnair`;
      case 7:
        return `${baseURL}/page8?${getUserdata?.user_id}&edit&questionnair`;
      // Add more cases as needed
      default:
        return `${baseURL}/page-default?${getUserdata}&edit`;
    }
  };
  const xyz =
    getUserdata?.is_additional_user == true
  ...
  const CustomToastpage2 = () => (
    <div className="Custom_tost">
      <div>
        <span style={{ color: "green", fontSize: "16px", marginRight: "8px" }}>
          <CheckOutlined />
        </span>
  ...
  const meeting_review_month: any = dayjs(
    getUserdata?.meetings?.lastMeeting?.start_meeting_date
  ).format("MM");
  const season_review_month = getSeasonByReviewMonth(meeting_review_month);
  console.log(season_review_month, "season");

  const meeting_review_year = dayjs(
    getUserdata?.meetings?.lastMeeting?.start_meeting_date
  ).format("YYYY");
  console.log(meeting_review_year, "meeting_review_year");

  const meeting_prepration_month: any = dayjs(
    getUserdata?.meetings?.NextMeeting?.start_meeting_date
  ).format("MM");
  const season_prepration_month = getSeasonByReviewMonth(
    meeting_prepration_month
  );
  console.log(season_prepration_month, "season_prepration_month");

  const meeting_prepration_year = dayjs(
    getUserdata?.meetings?.NextMeeting?.start_meeting_date
  ).format("YYYY");
              ...
              <Col span={24}>
                <Card className="common-card">
                  <div>
                    ...
                    {value == "page2" ? showToast && <CustomToastpage2 /> : ""}
                    {value == "page3" ? showToast && <CustomToastpage3 /> : ""}
                    {value == "page4" ? showToast && <CustomToastpage4 /> : ""}
                    {value == "page5" ? showToast && <CustomToastpage5 /> : ""}
                    {value == "page6" ? showToast && <CustomToastpage6 /> : ""}
                    {value == "page7" ? showToast && <CustomToastpage7 /> : ""}
                    {value == "additionalPage"
                      ? showToast && <CustomToastAdditonalPage />
                      : ""}
                    {value == "page8" ? showToast && <CustomToastPgae8 /> : ""}
                    <Row gutter={[20, 20]}>
                      {/* {data.map((item, index) => ( */}
                      {getUserdata?.template_access?.includes(
                        "bussiness_update"
                      ) && (
                        // <Col  span={8}>
                        ...
                        <Col xs={24} sm={12} md={12} lg={12} xl={8}>
                          ...
                          >
                            <div className="card-body pb-2 d-flex flex-column">
                              <div className="justify-content-between align-items-center d-flex">
                                <h5 className="fw-bold text-start mb-4">
                                  {season_review_month} {meeting_review_year}{" "}
                                  MEETING REVIEW
                                </h5>
                        ...
                        <Col xs={24} sm={12} md={12} lg={12} xl={8}>
                          ...
                          >
                            <div className="card-body pb-2 d-flex flex-column">
                              <div className="justify-content-between align-items-center d-flex">
                                <h5 className="fw-bold text-start mb-4">
                                  {season_prepration_month}{" "}
                                  {meeting_prepration_year} MEETING PREPARATION
                                </h5>
                                {state1?.roundTableTopics?.length ? (
                                  <Tooltip title="Download Pdf">
                                    <Button
                                      className="borderBtn"
                                      onClick={downLoadPdf6}
                                    >
                                      <DownloadOutlined />
                                    </Button>
                                  </Tooltip>
                          ...
                          >
                            <div className="card-body pb-2 d-flex flex-column">
                              <div className="justify-content-between align-items-center d-flex">
                                ...
                                {state1?.photo_section?.fileUrls?.length ? (
                                  <Tooltip title="Download Pdf">
                                    <Button
                                      className="borderBtn"
                                      onClick={downLoadPdf7}
                                    >
                                      <DownloadOutlined />
                                    </Button>
                                  </Tooltip>
                              ...
                              <div className="justify-content-between align-items-center d-flex">
                                ...
                                {state1?.craftsMenUpdates?.length ? (
                                  <Tooltip title="Download Pdf">
                                    <Button
                                      className="borderBtn"
                                      onClick={downLoadPdfBusinessEvolution}
                                    >
                                      <DownloadOutlined />
                                    </Button>
                                  </Tooltip>

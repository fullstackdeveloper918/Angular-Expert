...
const MeetingView = () => {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      let res: any = await axios.post(
        "https://frontend.goaideme.com/reset-password",
  ...
  const generatePdf = async () => {
             const companyName = getDisplayNameByKey(state?.company_name);

    console.log(state,"set state data show")
    const timestamp = new Date().toISOString().replace(/[-T:\.Z]/g, "");
    const blob = await pdf(<Pdf state={state} companyName={companyName} />).toBlob();
  ...
  const sharePdf = async () => {
    ...
    const file = new File([blob], `Detail_${timestamp}.pdf`, {
  ...
  const companyName = getDisplayNameByKey(
    state?.company_name || state?.master_user_detail?.company_name
  );
          ...
          >
            <Card className="common-card">
              ...
              <div className="d-flex justify-content-between">
                ...
                <div className=" ">
                  <Tooltip title="Download Pdf">
                    <Button className="ViewMore " onClick={downLoadPdf}>
                      {loadingState ? <Spin /> : <DownloadOutlined />}
                    </Button>
                  </Tooltip>

                  <Tooltip title="Share Pdf link">
                    <Button className="ViewMore " onClick={sharePdf}>
                      <ShareAltOutlined />
                    </Button>
                  </Tooltip>
                </div>
              ...
              <div className="card-listing">
                {state ? (
                  <div
                    className="card-listing-image my-5"
                    style={{
                      display: "flex",
                ...
                <ul className="list-unstyled my-4 mb-4">
                  <li className="mb-2">
                    ...
                    <Typography.Text className="ms-1 text-capitalize">
                      {state?.firstname
                  ...
                  <li className="mb-2">
                    ...
                    <Typography.Text className="ms-1">
                      {state?.company_name || "N/A"}
                    </Typography.Text>
                  </li>
                  <li className="mb-2">
                  ...
                  {getUserdata?.parent_user_id ? (
                    ...
                    <li className="mb-2">
                      ...
                      <Typography.Text className="ms-10">
                  ...
                  {getUserdata?.parent_user_id ? (
                    ...
                    <li className="mb-2">
                      ...
                      <Typography.Text className="ms-1">
                        {validation.capitalizeFirstLetter(state?.position) ||
                          "N/A"}
                      </Typography.Text>
                    </li>
                  )}
                  <li className="mb-2">
                    <Typography.Text>Home City:</Typography.Text>{" "}
                    <Typography.Text className="ms-1">
                      {validation.capitalizeFirstLetter(
                        state?.home_city ||
                          getUserdata?.master_user_detail?.home_city
                      ) || "N/A"}
                    </Typography.Text>
                  </li>
                </ul>
                ...
                {getUserdata?.parent_user_id ? (
                  ...
                  <div className="card-listing-button d-inline-flex flex-wrap gap-3 w-100 cardButtons">
                    {getUserdata?.is_admin == true ? (
                      <Link
                        href={`/admin/member/add?${state?.uid}&edit`}
                    ...
                    {!getUserdata?.is_admin == false ? (
                      <Popconfirm
                        title={`${
                          state?.is_activate ? "Deactivate" : "Activate"
                        } the club member`}
                      ...
                      >
                        <Button
                          ...
                          type="primary"
                          htmlType="button"
                          className="flex-grow-1 activateBtn"
                          ghost
                        >
                          {" "}
                          {state?.is_activate ? "Activate" : "Deactivate"}
                        </Button>
                      </Popconfirm>
                    ) : (
                      <Button
                    ...
                    {getUserdata?.is_admin == true ? (
                      <Button
                        ...
                        type="primary"
                        htmlType="button"
                        className="flex-grow-1 w-100 primaryBtn"
                        loading={loading}
                        onClick={onFinish}
                      >
                        Reset Password
                    ...
                    {!getUserdata?.is_admin == false ? (
                      ...
                      >
                        <Button
                          ...
                          type="primary"
                          htmlType="button"
                          className="flex-grow-1 w-100 archiveBtn"
                          danger
                        >
                          Archive
                        </Button>

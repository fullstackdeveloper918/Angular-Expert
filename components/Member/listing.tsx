...
const MemberList = ({ response }: any) => {
  ...
  const sharePdf = async (item: any) => {
    //
    const companyName = companyNameData[item?.company_name || ""] || "N/A";
    const { pdfUrl, timestamp } = await generatePdf(item);
    const response = await fetch(pdfUrl);
    const blob = await response.blob();

    // Convert the blob to a file
    const file = new File([blob], `${capFirst(item?.company_name)}.pdf`, {
        ...
        <ul className="m-0 list-unstyled d-flex gap-2">
          <li>
            <Tooltip title="Download Pdf">
              <Button
                className="ViewMore "
                onClick={() => handleDownloadAndFetchData(res?.uid)}
              >
                {" "}
          ...
          <li>
            <Tooltip title="Share Pdf link">
              <Button
                className="ViewMore "
                onClick={() => handleFetchAndFetchData(res?.uid)}
              >
                <ShareAltOutlined />
              </Button>
            </Tooltip>
          ...
          <li>
            <Link href={`/admin/member/${res?.uid}/view`}>
              {" "}
  ...
  const dataSource = filteredData?.map((res: any, index: number) => {
    ...
    console.log(res.company_name, "here to eeema a");
        ...
        <ul className="m-0 list-unstyled d-flex gap-2">
          ...
          <li>
            <Tooltip title="Share Pdf link">
              <Button
                className="ViewMore "
                onClick={() => handleFetchAndFetchData(res?.id)}
              >
                <ShareAltOutlined />
              </Button>
            </Tooltip>
  ...
  const getData = async (query: string, lastVisibleId?: string) => {
    ...
    try {
      ...
      setState1(res?.data || []);
      console.log(res?.data, "mochup devele");
      ...
      console.log(apiRes?.data, "mochup devele 222");
                  ...
                  {loading1 ? (
                    ...
                    <>
                      {getUserdata?.is_admin == false ? (
                        <Table
                          className="tableBox"
                          dataSource={user_completed}
                          columns={user_completed_columns}
                          pagination={{

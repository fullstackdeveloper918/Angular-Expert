...
const Page7 = ({ questions }: any) => {
  ...
  const filteredQuestions = questions?.data
    ...
    .sort(
      (a: any, b: any) =>
        parseInt(a.question_position) - parseInt(b.question_position)
    )
    .map((item: any, index: number) => ({
      ...item,
      question_position: index.toString(),
    }));
  ...
  const getDataById = async () => {
    ...
    try {
      ...
      resValues["estimating"] =
        dataFromApi?.estimating ?? formValues?.estimating ?? "";
      resValues["accountability"] =
        dataFromApi?.accountability ?? formValues?.accountability ?? "";

      // Check if `dataFromApi.productivity` has a valid value before using `formValues.productivity`
      resValues["productivity"] =
        dataFromApi?.productivity && dataFromApi?.productivity.trim() !== ""
  ...
  const sendData = async (items: any, isEdit: boolean) => {
    ...
    try {
      const res = !pagetype
        ? await api.User.edit(items)
        : await api.Auth.signUp(items);
      return res;
    } catch (error) {
              ...
              <div className="card-form-wrapper">
                ...
                >
                  <Typography.Title level={5}>
                    LIST THREE ROUNDTABLE TOPICS THAT YOU WANT TO COVER IN OUR
                    SPRING MEETING (IN ORDER OF IMPORTANCE)
                  </Typography.Title>
                  <Divider plain />
